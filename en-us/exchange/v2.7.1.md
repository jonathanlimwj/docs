...
NEO has two types of digital assets: global assets (e.g. NEO, GAS) and contract assets (e.g. NEP-5 assets). The global assets are managed using the Unspent Transaction Output (UTXO) model, whereas the contract assets are managed using the BALANCE model. That's the main assets exchanges need to deal with for user withdrawals,  deposits, and other operations. The NEO-CLI client works as an normal node in the P2P network and meanwhile a cross-platform wallet handling various assets related transactions. 
